...
核心能力：Fast Gate -> Full Gate、PR-aware concurrency、path-aware heavy builds、Governor `normal cancel -> recheck -> force-cancel`、jobless ghost run 清理、失败分类、受限 Recovery、AI Repair Brief、Single Release Authority 和 Node24-native action baseline。
...
确定性测试/编译失败不靠 rerun 解决；瞬时 Runner/网络/GitHub 5xx 最多受限重跑一次；ghost run 不进入代码 Recovery；Release/Deploy/Publish 禁止盲目 replay。
