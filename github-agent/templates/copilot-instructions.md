...
- Read `/.github/GITHUB_AGENT.md`, `/.github/ACTIONS_STRATEGY.md`, and `/AGENTS.md` first when present.
- Check open `[GitHub Agent][AI Repair]` Issues and then inspect the referenced full Run logs and source commit.
- Distinguish deterministic test/config failures from transient Runner/network failures and jobless ghost runs.
- Never rerun deterministic assertion/compile/lint failures merely to make the red status disappear.
...
- Use a dedicated repair branch/PR, run the original failing test plus regressions, and preserve GitHub Agent Policy Check compliance.
